...
namespace OpenEQ.FileConverter.Entities
{
    public class FragMesh
    {
        ...
        public Tuple<bool, Vector3>[] Polys;
        ...
        public FragMesh(BinaryReader input, bool isOldVersion, FragRef[] textures, string name)
        {
            ...
            // Skipping min
            input.ReadSingle(3);//var min = new Vector3(input.ReadSingle(), input.ReadSingle(), input.ReadSingle());

            // Skipping max
            input.ReadSingle(3);//var max = new Vector3(input.ReadSingle(), input.ReadSingle(), input.ReadSingle());
            ...
            Polys = new Tuple<bool, Vector3>[polycount];
            for (var i = 0; i < polycount; i++)
            {
                Polys[i] = new Tuple<bool, Vector3>(input.ReadUInt16() != 0x0010,
                    new Vector3(input.ReadUInt16(), input.ReadUInt16(), input.ReadUInt16()));
            }

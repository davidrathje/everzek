...
namespace OpenEQ.FileConverter.Entities
{
    public class Mesh
    {
        ...
        public List<Tuple<bool, Vector3>> Polygons;


        public Mesh(Material material, VertexBuffer vbuf, List<Tuple<bool, Vector3>> poly)
        {
            Material = material;
            VertexBuffer = vbuf;
            Polygons = poly;
        ...
        public void Add(Mesh mesh)
        {
            ...
            if (!VertexBuffer.Equals(mesh.VertexBuffer))
            {
                ...
                {
                    Polygons.Add(new Tuple<bool, Vector3>(poly.Item1,
                        new Vector3(poly.Item2.x + offset, poly.Item2.y + offset, poly.Item2.z + offset)));
                }
            }
            else
            {
        ...
        public Mesh Subset(List<Tuple<bool, Vector3>> cpoly)
        {
            var vbuffer = new List<float>();
            var npoly = new List<Tuple<bool, Vector3>>();
            var used = new Dictionary<float, int>();

            foreach (var poly in cpoly)
            {
                npoly.Add(new Tuple<bool, Vector3>(poly.Item1,
                    new Vector3(MapIndex(poly.Item2.x, used, vbuffer),
        ...
        public List<Mesh> Optimize()
        {
            ...
            var cpoly = new List<Tuple<bool, Vector3>>();
